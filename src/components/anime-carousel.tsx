  ...
  return (
    <div className={cn(["flex flex-col gap-4", props.className])}>
      <div className="w-full flex items-center justify-between">
        <h5 className="text-xl sm:text-2xl font-bold section-header">{props.title}</h5>
        <div className="flex items-center gap-2">
          <Button
            onClick={() => { api?.scrollPrev(); }}
            className="rounded-full bg-transparent border border-white/20 h-8 w-8 hover:bg-white/10 hover:border-white/40 transition-all btn-press"
          >
            <ArrowLeft className="text-white shrink-0" size={14} />
          </Button>
          <Button
