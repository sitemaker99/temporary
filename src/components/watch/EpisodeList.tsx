...
export const EpisodeList: React.FC<Props> = ({
  ...
  const [displayMode, setDisplayMode] = useState<'list' | 'grid' | 'imageList'>(
    () => {
      const savedMode =
        typeof window !== "undefined" && animeId
  ...
  // Load episode progress percentages from localStorage
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const allTimes = JSON.parse(localStorage.getItem("all_episode_times") || "{}");
      const progress: Record<string, number> = {};
      Object.entries(allTimes).forEach(([epId, data]: [string, any]) => {
        if (data?.playbackPercentage) {
          progress[epId] = Math.min(Math.round(data.playbackPercentage), 100);
        }
      });
  ...
  // Dynamic max height calculation — SSR-safe
  const dynamicMaxHeight = () => {
    if (typeof window === "undefined") return maxListHeight;
    ...
    >
      ...
      >
        {displayedEpisodes.map((episode) => {
          ...
          const isRowDisplayMode = displayMode === 'list' || displayMode === 'imageList';

          // Dynamic button classes based on state
          const getButtonClasses = () => {
            const baseClasses = "rounded-sm border hover:border-primary transition-colors flex items-center justify-center";
            ...
            if (isSelected) {
              ...
              return `${baseClasses} bg-primary text-primary-foreground brightness-75 hover:brightness-110`;
            } else {
              return `${baseClasses} bg-muted text-muted-foreground hover:bg-accent hover:text-accent-foreground hover:brightness-105`;
            }
          };

          const paddingClasses = isRowDisplayMode ? 'p-2' : 'p-1';
          const textAlignClasses = isRowDisplayMode ? 'text-left' : 'text-center';
          const flexClasses = isRowDisplayMode ? 'justify-between' : 'justify-center';
          ...
          return (
            ...
            >
              {displayMode === 'imageList' ? (
                ...
                // List layout
                <div className="flex flex-col w-full text-left px-1 gap-0.5">
                  <div className="flex justify-start gap-2 items-center w-full">
                    <span>{isSelected ? <FaPlay /> : (episode.number + ".")}</span>
                    <span className="p-0 truncate">{episode.title}</span>
                  </div>
                  {episodeProgress[episode.id] > 0 && (
                    <div className="w-full h-0.5 bg-white/10 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-red-500 rounded-full"
                        style={{ width: `${episodeProgress[episode.id]}%` }}
                      />
                    </div>
                  )}
                </div>
