...
export function Player({
  ...
  const getIframeSrc = () => {
    // Handle all hianime episodeId formats:
    // "anime-name?ep=12345", "anime-name$ep=12345", "anime-name$12345$12345"
    const epMatch = episodeId.match(/[?&$]?ep[=:](\d+)/i) || episodeId.match(/\$(\d+)$/) || episodeId.match(/^(\d+)$/);
    const extractedEpId = epMatch ? epMatch[1] : episodeId;
    const normalizedServer = (serverName || "hd-1").toLowerCase();
    const domain =
      normalizedServer === "hd-2" || normalizedServer === "vidstreaming" || normalizedServer === "megacloud"
        ? "vidwish.live"
        : "megaplay.buzz";
    const lang = category === "dub" ? "dub" : "sub";
    return `https://${domain}/stream/s-2/${extractedEpId}/${lang}`;
  };
